...
def ensure_admin():
    users = load_users()
    if not any(u["username"]=="Saltpapi" for u in users):
        users.append({"username":"Saltpapi","password":"Saltpapi1"})
        save_users(users)
    ...
    if request.method=="POST":
        ...
        for u in users:
            if u["username"]==username and u["password"]==password:
                ...
                return redirect(url_for("dashboard") if username != "Saltpapi" else url_for("admin"))
...
def console(owner, server_name):
    ...
    if session["username"] != owner and session["username"] != "Saltpapi":
...
def upload_file_route(owner, server_name):
    if "username" not in session or (session["username"] != owner and session["username"] != "Saltpapi"):
...
def admin():
    if "username" not in session or session["username"]!="Saltpapi":
        return redirect(url_for("login"))
